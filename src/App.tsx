...
export default function App() {
  ...
  const initWebSocket = async (token: string) => {
    ...
    try {
      ...
      const socket = new WebSocket(`${protocol}//${window.location.host}/ws?ticket=${ticket}`);
    ...
    <div className="min-h-screen bg-[#0f0a1e] flex items-center justify-center p-4">
      <div className="glass p-10 max-w-md w-full rounded-[40px] shadow-2xl relative overflow-hidden">
        ...
        <form className="space-y-5" onSubmit={(e) => { e.preventDefault(); if (mode === 'register') onRegister(name, email, phone, password); else onLogin(phone, password); }}>
          ...
          <input type="password" autoComplete={mode === 'login' ? "current-password" : "new-password"} placeholder={t('auth.password')} value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-6 py-4 rounded-2xl bg-white/5 border border-white/10 text-white placeholder-white/20 focus:ring-2 focus:ring-purple-500 outline-none transition-all" required />
